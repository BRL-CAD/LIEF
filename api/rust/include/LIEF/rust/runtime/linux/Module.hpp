...
class runtime_linux_Module : public runtime_Module {
  public:
  using lief_t = LIEF::runtime::Linux::Module;
  runtime_linux_Module(std::unique_ptr<lief_t> mod) :
  ...
  static auto from_handle(uint64_t handle) {
    return details::try_unique<runtime_linux_Module>(
...
inline auto runtime_linux_dlopen(const std::string& name) {
  return details::try_unique<runtime_linux_Module>(
      LIEF::runtime::Linux::dlopen(name)
  );
}
