...
namespace LIEF::PE {
std::unique_ptr<CHPEMetadataX86> CHPEMetadataX86::parse(Parser& /*ctx*/,
                                                        BinaryStream& stream,
                                                        uint32_t version) {
  ...
  if (!CHPECodeAddressRangeOffset) {
    LIEF_DEBUG(
        "Failed to read Compiled Hybrid Metadata[X86]: CHPECodeAddressRangeOffset"
    );
    return nullptr;
  }
  ...
  if (!CHPECodeAddressRangeCount) {
    LIEF_DEBUG(
        "Failed to read Compiled Hybrid Metadata[X86]: CHPECodeAddressRangeCount"
    );
    return nullptr;
  }
  ...
  if (!WowA64ExceptionHandlerFunctionPointer) {
    LIEF_DEBUG("Failed to read Compiled Hybrid Metadata[X86]: "
  ...
  if (!WowA64RdtscFunctionPointer) {
    LIEF_DEBUG(
        "Failed to read Compiled Hybrid Metadata[X86]: WowA64RdtscFunctionPointer"
    );
    return metadata;
  }
  return metadata;
