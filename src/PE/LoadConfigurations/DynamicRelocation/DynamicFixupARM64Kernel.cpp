...
//  +00,00 | ULONG PageRelativeOffset : 10;  // Offset to the call instruction
//  shifted right by 2 (4-byte aligned instruction) +10,0A | ULONG IndirectCall :
//  1;  // 0 if target instruction is a BR, 1 if BLR. +11,0B | ULONG RegisterIndex
//  :  5;  // Register index used for the indirect call/jump. +16,10 | ULONG
//  ImportType         :  1;  // 0 if this refers to a static import, 1 for
//  delayload import +17,11 | ULONG IATIndex           : 15;  // IAT index of the
//  corresponding import.
//                                     // 0x7FFF is a special value indicating no
//                                     index.
...
std::string DynamicFixupARM64Kernel::reloc_entry_t::to_string() const {
  return fmt::format(
      "RVA: {:#010x} Instr: {:8} delayload: {} IAT index: {:04d}", rva,
      fmt::format("{} x{}", indirect_call ? "blr" : "br", register_index),
...
std::unique_ptr<DynamicFixupARM64Kernel>
  ...
  while (strm) {
    ...
    while (*block_strm) {
      ...
      LIEF_DEBUG("PAGE={:#012x} RVA: {:#08x} IndirectCall: {}, RegIndex: {}, "
                 "ImportType: {}, "
                 "IATIndex: {}",
