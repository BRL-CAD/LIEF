...
void Section::name(std::string name) {
  if (name.size() > MAX_SECTION_NAME) {
    LIEF_ERR("Section name too long: max {} vs {}", MAX_SECTION_NAME, name.size());
    return;
  }
  name_ = std::move(name);
}
...
std::vector<Section::CHARACTERISTICS>
    Section::characteristics_to_list(uint32_t value) {
...
const char* to_string(Section::CHARACTERISTICS e) {
  CONST_MAP(Section::CHARACTERISTICS, const char*, 35)
  enumStrings{
