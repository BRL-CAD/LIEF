...
ok_error_t Parser::parse_binary() {
  ...
  if (DynamicEntry* dt_hash = binary_->get(DynamicEntry::TAG::HASH)) {
    if (auto res = binary_->virtual_address_to_offset(dt_hash->value())) {
      ...
      LIEF_WARN("Can't convert DT_HASH.virtual_address into an offset ({:#x})",
                dt_hash->value());
    }
  }
...
ok_error_t Parser::parse_dyn_table(Segment& pt_dyn) {
  ...
  LIEF_DEBUG("Dynamic content wrapped by segment LOAD: [{:#018x}, {:#018x}] "
             "[{:#018x}, {:#018x}]",
             load_seg.virtual_address(),
             load_seg.virtual_address() + load_seg.virtual_size(),
             load_seg.file_offset(),
             load_seg.file_offset() + load_seg.physical_size());
...
ok_error_t Parser::process_dynamic_table() {
  {
    ...
    if (dt_syment != nullptr &&
        dt_syment->value() != sizeof(typename ELF_T::Elf_Sym))
    {
    ...
    if (dt_symtab != nullptr && config_.parse_dyn_symbols) {
      ...
      if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
        ...
        LIEF_WARN("Can't convert DT_SYMTAB.virtual_address into an offset ({:#x})",
                  virtual_address);
      }
    }
  }
  {
    ...
    if (dt_rela != nullptr && dt_relasz != nullptr && config_.parse_relocations) {
      ...
      if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
        ...
        LIEF_WARN("Can't convert DT_RELA.virtual_address into an offset ({:#x})",
                  virtual_address);
      }
    }
  }
  {
    ...
    if (dt_rel != nullptr && dt_relsz != nullptr && config_.parse_relocations) {
      ...
      if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
        ...
        LIEF_WARN("Can't convert DT_REL.virtual_address into an offset ({:#x})",
                  virtual_address);
      }
    }
  }
  {
    ...
    if (dt_relr != nullptr && dt_relrsz != nullptr && config_.parse_relocations) {
      ...
      if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
        ...
        LIEF_WARN("Can't convert DT_RELR.virtual_address into an offset ({:#x})",
                  virtual_address);
      }
    }
  }
  {
    ...
    if (dt_relr != nullptr && dt_relrsz != nullptr && config_.parse_relocations) {
      ...
      if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
        ...
        LIEF_WARN("Can't convert (Android)DT_RELR.virtual_address into an offset "
                  "({:#x})",
                  virtual_address);
      }
    }
  }
  {
    ...
    if (dt_rela != nullptr && dt_relasz != nullptr && config_.parse_relocations) {
      ...
      if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
        ...
        LIEF_WARN("Can't convert DT_ANDROID_REL[A].virtual_address into an offset "
                  "({:#x})",
                  virtual_address);
      }
    }
  }
  {
        ...
        config_.parse_relocations)
    {
      ...
      if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
        ...
        LIEF_WARN("Can't convert DT_JMPREL.virtual_address into an offset ({:#x})",
                  virtual_address);
      }
    }
  }
  if (config_.parse_symbol_versions && config_.parse_dyn_symbols) {
    if (DynamicEntry* dt_versym = binary_->get(DynamicEntry::TAG::VERSYM)) {
      ...
      if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
        ...
        binary_->sizing_info_->versym =
            binary_->dynamic_symbols_.size() * sizeof(uint16_t);
      } else {
        LIEF_WARN("Can't convert DT_VERSYM.virtual_address into an offset ({:#x})",
                  virtual_address);
      }
    }
  }
  ...
  if (config_.parse_symbol_versions) {
    ...
    if (dt_verneed != nullptr && dt_verneed_num != nullptr) {
      ...
      if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
        ...
        LIEF_WARN(
            "Can't convert DT_VERNEED.virtual_address into an offset ({:#x})",
            virtual_address
        );
      }
    }
...
template<typename ELF_T>
result<uint32_t>
    Parser::get_numberof_dynamic_symbols(ParserConfig::DYNSYM_COUNT mtd) const {
  ...
  switch (mtd) {
    ...
    case ParserConfig::DYNSYM_COUNT::RELOCATIONS:
    ...
    default:
    {
      ...
      if (auto res = get_numberof_dynamic_symbols<ELF_T>(
              ParserConfig::DYNSYM_COUNT::RELOCATIONS
          ))
      {
        nb_reloc = *res;
      }
      ...
      if (auto res = get_numberof_dynamic_symbols<ELF_T>(
              ParserConfig::DYNSYM_COUNT::SECTION
          ))
      {
        nb_section = *res;
      }
      ...
      if (auto res = get_numberof_dynamic_symbols<ELF_T>(
              ParserConfig::DYNSYM_COUNT::HASH
          ))
      {
        nb_hash = *res;
      }
      ...
      if (nb_section < Parser::NB_MAX_SYMBOLS && nb_section > nb_reloc &&
      ...
      if (nb_hash < Parser::NB_MAX_SYMBOLS && nb_hash > candidate &&
...
result<uint32_t> Parser::nb_dynsym_relocations() const {
  ...
  if (dt_rela != nullptr && dt_relasz != nullptr) {
    ...
    if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
      nb_symbols =
          std::max(nb_symbols, max_relocation_index<ELF_T, rela_t>(*res, size));
    }
  }
  ...
  if (dt_rel != nullptr && dt_relsz != nullptr) {
    ...
    if (auto res = binary_->virtual_address_to_offset(virtual_address)) {
      nb_symbols =
          std::max(nb_symbols, max_relocation_index<ELF_T, rel_t>(*res, size));
    }
  }
...
ok_error_t Parser::parse_sections() {
  ...
  for (std::unique_ptr<Section>& section : binary_->sections_) {
    ...
    if (it_name_idx == sections_names.end()) {
      LIEF_WARN("Missing name_idx for section at offset {:#x}",
                section->file_offset());
      continue;
    }
    ...
    if (!name) {
      LIEF_ERR("Can't read section name for section {:#x}",
               section->file_offset());
      break;
    }
    section->name(*name);
  ...
  for (size_t i = 0; i < nbof_segments; ++i) {
        ...
        segment->physical_size() < Parser::MAX_SEGMENT_SIZE)
    {
      ...
      if (read_size > Parser::MAX_SEGMENT_SIZE) {
        LIEF_WARN("Segment #{} is {} bytes large. Only the first {} bytes will be "
                  "taken into account",
                  i, read_size, Parser::MAX_SEGMENT_SIZE);
        read_size = Parser::MAX_SEGMENT_SIZE;
      }
      if (read_size > stream_->size()) {
        LIEF_WARN("Segment #{} has a physical size larger than the current stream "
                  "size ({} > {}). "
...
ok_error_t Parser::parse_packed_relocations(uint64_t offset, uint64_t size) {
  ...
  while (nb_relocs > 0) {
    ...
    uint64_t groupr_info = g_by_info ? rel_stream->read_sleb128().value_or(0) : 0;
    ...
    for (size_t i = 0; i < nb_reloc_group; ++i) {
      ...
      r_offset += g_by_offset_delta ? group_off_delta :
                                      rel_stream->read_sleb128().value_or(0);
      uint64_t info =
          g_by_info ? groupr_info : rel_stream->read_sleb128().value_or(0);
...
ok_error_t Parser::parse_relative_relocations(uint64_t offset, uint64_t size) {
  ...
  switch (arch) {
    case ARCH::AARCH64: type = Relocation::TYPE::AARCH64_RELATIVE; break;
    case ARCH::X86_64: type = Relocation::TYPE::X86_64_RELATIVE; break;
    case ARCH::ARM: type = Relocation::TYPE::ARM_RELATIVE; break;
    case ARCH::HEXAGON: type = Relocation::TYPE::HEX_RELATIVE; break;
    case ARCH::PPC64: type = Relocation::TYPE::PPC64_RELATIVE; break;
    case ARCH::PPC: type = Relocation::TYPE::PPC_RELATIVE; break;
    case ARCH::I386:
    case ARCH::IAMCU: type = Relocation::TYPE::X86_RELATIVE; break;
    default: break;
  }
...
ok_error_t Parser::parse_dynamic_symbols(uint64_t offset) {
  ...
  if (!res) {
    LIEF_ERR("Fail to get the number of dynamic symbols with the current counting "
             "method");
  ...
  for (size_t i = 0; i < nb_symbols; ++i) {
    ...
    auto symbol =
        std::unique_ptr<Symbol>(new Symbol(std::move(*symbol_header),
                                           binary_->header().machine_type()));
  ...
  binary_->sizing_info_->dynsym =
      binary_->dynamic_symbols_.size() * sizeof(Elf_Sym);
...
ok_error_t Parser::parse_dynamic_entries(BinaryStream& stream) {
  ...
  max_nb_entries =
      std::min<uint32_t>(max_nb_entries, Parser::NB_MAX_DYNAMIC_ENTRIES);
  ...
  if (DynamicEntry* dt_init_array = binary_->get(DynamicEntry::TAG::INIT_ARRAY)) {
    if (DynamicEntry* dt_init_arraysz =
            binary_->get(DynamicEntry::TAG::INIT_ARRAYSZ))
    {
      ...
      LIEF_WARN("The binary is not consistent. Found DT_INIT_ARRAY but missing "
                "DT_INIT_ARRAYSZ");
    }
  }
  ...
  if (DynamicEntry* dt_fini_array = binary_->get(DynamicEntry::TAG::FINI_ARRAY)) {
    if (DynamicEntry* dt_fini_arraysz =
            binary_->get(DynamicEntry::TAG::FINI_ARRAYSZ))
    {
      ...
      LIEF_WARN("The binary is not consistent. Found DT_FINI_ARRAY but missing "
                "DT_FINI_ARRAYSZ");
    }
  }
  ...
  // =======================
  if (DynamicEntry* dt_preini_array =
          binary_->get(DynamicEntry::TAG::PREINIT_ARRAY))
  {
    if (DynamicEntry* dt_preinit_arraysz =
            binary_->get(DynamicEntry::TAG::PREINIT_ARRAYSZ))
    {
      ...
      LIEF_WARN("The binary is not consistent. Found DT_PREINIT_ARRAY but missing "
                "DT_PREINIT_ARRAYSZ");
    }
  }
  return ok();
...
struct RelocationSetEq {
  bool operator()(const Relocation* lhs, const Relocation* rhs) const {
    bool check = lhs->address() == rhs->address() && lhs->type() == rhs->type() &&
                 lhs->addend() == rhs->addend() && lhs->info() == rhs->info() &&
...
ok_error_t Parser::parse_section_relocations(const Section& section) {
  ...
  while (reloc_stream) {
    ...
    const bool is_from_dynsym =
        ...
        (symbol_table == nullptr || symbol_table->type() == Section::TYPE::DYNSYM);

    const bool is_from_symtab =
        idx < binary_->symtab_symbols_.size() &&
        (symbol_table == nullptr || symbol_table->type() == Section::TYPE::SYMTAB);
    if (is_from_dynsym) {
...
ok_error_t Parser::parse_symbol_version_requirement(uint64_t offset,
  ...
  for (size_t sym_idx = 0; sym_idx < nb_entries; ++sym_idx) {
    const auto header =
        stream_->peek<Elf_Verneed>(svr_offset + next_symbol_offset);
    if (!header) {
      break;
    }
...
ok_error_t Parser::parse_symbol_version_definition(uint64_t offset,
  ...
  for (size_t i = 0; i < nb_entries; ++i) {
    ...
    def_size =
        std::max(def_size, verdef_stream->pos() - offset + sizeof(Elf_Verdef));
    if (!svd_header) {
      break;
    }
    ...
    uint32_t nb_aux_symbols = svd_header->vd_cnt;
    {
      ...
      for (size_t j = 0; j < nb_aux_symbols; ++j) {
        ...
        def_size =
            std::max(def_size, aux_stream->pos() - offset + sizeof(Elf_Verdaux));
        if (!svda_header) {
          break;
        }
