...
ok_error_t BinaryParser::parse() {
   ...
   * the exports trie as it could create new symbols and break the
   * DynamicSymbolCommand's indexes
   */
...
ok_error_t BinaryParser::parse_load_commands() {
  ...
  for (size_t i = 0; i < nbcmds; ++i) {
    ...
    LIEF_DEBUG("Parsing command #{:02d}: {} ({:#06x})", i, to_string(cmd_type),
               (uint64_t)cmd_type);
    ...
    switch (cmd_type) {
      ...
      {
        const auto cmd =
            stream_->peek<details::routines_command_64>(loadcommands_offset);
        if (!cmd) {
      ...
      {
        const auto cmd =
            stream_->peek<details::routines_command_32>(loadcommands_offset);
        if (!cmd) {
      ...
      case LoadCommand::TYPE::SUB_CLIENT:
      {
         ...
         */
        const auto cmd =
            stream_->peek<details::sub_client_command>(loadcommands_offset);
        if (!cmd) {
      ...
      case LoadCommand::TYPE::ENCRYPTION_INFO_64:
      {
        ...
        LIEF_DEBUG("[+] Parsing {}", to_string(cmd_type));
        const auto cmd =
            stream_->peek<details::encryption_info_command>(loadcommands_offset);
        if (!cmd) {
      ...
      case LoadCommand::TYPE::DYLD_CHAINED_FIXUPS:
      {
        ...
        SegmentCommand* lnk =
            config_.from_dyld_shared_cache ?
        ...
        if (lnk == nullptr) {
          LIEF_WARN("Can't find the segment associated with "
                    "the LC_DYLD_CHAINED_FIXUPS payload (offset: {:#x})",
                    chained->data_offset());
          break;
        }
        ...
        if ((chained->data_offset() + chained->data_size()) >
            (lnk->file_offset() + content.size()))
        {
          LIEF_WARN(
              "LC_DYLD_CHAINED_FIXUPS payload does not fit in the '{}' segments",
              lnk->name()
          );
      ...
      {
        if (const auto cmd = stream_->peek<details::twolevel_hints_command>(
                loadcommands_offset
            ))
        {
      ...
      case LoadCommand::TYPE::LINKER_OPTIMIZATION_HINT:
      {
                ...
                stream_->peek<details::linkedit_data_command>(loadcommands_offset))
        {
          ...
          LIEF_ERR(
              "Can't parse linkedit_data_command for LC_LINKER_OPTIMIZATION_HINT"
          );
        }
        break;
      }
      ...
      {
        if (const auto cmd =
                stream_->peek<details::note_command>(loadcommands_offset))
        {
      ...
      case LoadCommand::TYPE::FUNCTION_VARIANT_FIXUPS:
      {
        ...
        if (!cmd) {
          LIEF_ERR(
              "Can't parse linkedit_data_command for LC_FUNCTION_VARIANT_FIXUPS"
          );
          break;
        }
...
ok_error_t BinaryParser::parse_relocations(Section& section) {
  ...
  if (section.numberof_relocations() > BinaryParser::MAX_RELOCATIONS) {
    ...
    LIEF_WARN(
        "Huge number of relocations (#{:d}). Only the first #{:d} will be parsed",
  ...
  for (size_t i = 0; i < numberof_relocations; ++i) {
    ...
    if (is_scattered) {
          ...
          ))
      {
        ...
        LIEF_INFO("Can't read scattered_relocation_info for #{}@{:#x}", i,
                  current_reloc_offset);
        break;
      }
    } else {
      ...
      if (auto res = stream_->peek<details::relocation_info>(current_reloc_offset))
      {
        ...
        LIEF_INFO("Can't read relocation_info for #{}@{:#x}", i,
                  current_reloc_offset);
        break;
      }
      ...
      if (reloc_info.r_extern == 0) {
        ...
        if (sec_num == Relocation::R_ABS) {
           ...
           * }
           * findTargetFromAddressAndSectionNum can fail *silently* so no need to
  ...
  while (!done && stream_->pos() < end_offset) {
    ...
    switch (DyldInfo::REBASE_OPCODES(opcode)) {
      ...
      case DyldInfo::REBASE_OPCODES::SET_SEGMENT_AND_OFFSET_ULEB:
      {
        ...
        if (!seg_offset) {
          LIEF_ERR("REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB: Can't read uleb128 "
                   "offset");
          break;
        }
        segment_index = imm;
      ...
      case DyldInfo::REBASE_OPCODES::ADD_ADDR_IMM_SCALED:
      {
        ...
        if (current_segment == nullptr) {
          ...
          LIEF_WARN(
              "REBASE_OPCODE_ADD_ADDR_IMM_SCALED: Bad offset ({:#x} > {:#x})",
              segment_offset, current_segment->file_size()
          );
        }
        break;
      ...
      case DyldInfo::REBASE_OPCODES::DO_REBASE_IMM_TIMES:
      {
        for (size_t i = 0; i < imm; ++i) {
          ...
          if (current_segment == nullptr) {
            LIEF_WARN(
                "REBASE_OPCODE_DO_REBASE_IMM_TIMES: the current segment is null"
            );
          } else if (segment_offset > current_segment->file_size()) {
            LIEF_WARN(
                "REBASE_OPCODE_DO_REBASE_IMM_TIMES: Bad offset ({:#x} > {:#x})",
                segment_offset, current_segment->file_size()
            );
          }
        }
      ...
      case DyldInfo::REBASE_OPCODES::DO_REBASE_ULEB_TIMES:
      {
        ...
        for (size_t i = 0; i < count; ++i) {
          if (current_segment == nullptr) {
            LIEF_WARN(
                "REBASE_OPCODE_DO_REBASE_ULEB_TIMES: the current segment is null"
            );
          } else if (segment_offset > current_segment->file_size()) {
            LIEF_WARN(
                "REBASE_OPCODE_DO_REBASE_ULEB_TIMES: Bad offset ({:#x} > {:#x})",
                segment_offset, current_segment->file_size()
            );
          }
      ...
      case DyldInfo::REBASE_OPCODES::DO_REBASE_ADD_ADDR_ULEB:
      {
        if (current_segment == nullptr) {
          LIEF_WARN(
              "REBASE_OPCODE_DO_REBASE_ADD_ADDR_ULEB: the current segment is null"
          );
        } else if (segment_offset > current_segment->file_size()) {
          LIEF_WARN(
              "REBASE_OPCODE_DO_REBASE_ADD_ADDR_ULEB: Bad offset ({:#x} > {:#x})",
              segment_offset, current_segment->file_size()
          );
        }
        ...
        auto uleb128_val = stream_->read_uleb128();
        if (!uleb128_val) {
          LIEF_ERR("REBASE_OPCODE_DO_REBASE_ADD_ADDR_ULEB: Can't read uleb128 "
                   "segment_offset");
          break;
        }
      ...
      case DyldInfo::REBASE_OPCODES::DO_REBASE_ULEB_TIMES_SKIPPING_ULEB:
      {
        ...
        if (!uleb128_val) {
          LIEF_ERR(
              "Can't read REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB count"
          );
          break;
        }
        count = *uleb128_val;
        ...
        if (!uleb128_val) {
          LIEF_ERR(
              "Can't read REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB skip"
          );
          break;
        }
        // Skip
        ...
        for (size_t i = 0; i < count; ++i) {
          if (current_segment == nullptr) {
            LIEF_WARN("REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB: the "
                      "current segment is null");
          } else if (segment_offset > current_segment->file_size()) {
            LIEF_WARN("REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB: Bad "
                      "offset ({:#x} > {:#x})",
                      segment_offset, current_segment->file_size());
          }
          if (is_err(do_rebase<MACHO_T>(type, segment_index, segment_offset,
  ...
  while (!done && stream_->pos() < end_offset) {
    ...
    switch (opcode) {
      ...
      case DyldInfo::BIND_OPCODES::SET_SEGMENT_AND_OFFSET_ULEB:
      {
        auto val = stream_->read_uleb128();
        if (!val) {
          LIEF_ERR("Can't read BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB uleb128 "
                   "segment offset");
          break;
        }
        segment_idx = imm;
      ...
      case DyldInfo::BIND_OPCODES::DO_BIND_ADD_ADDR_ULEB:
      {
        ...
        if (!val) {
          LIEF_ERR(
              "Can't read BIND_OPCODE_DO_BIND_ADD_ADDR_ULEB uleb128 segment offset"
          );
          break;
        }
      ...
      case DyldInfo::BIND_OPCODES::DO_BIND_ULEB_TIMES_SKIPPING_ULEB:
      {
        auto val = stream_->read_uleb128();
        if (!val) {
          LIEF_ERR("Can't read BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB "
                   "uleb128 count");
          break;
        }
        count = *val;
          ...
          case DyldInfo::BIND_SUBOPCODE_THREADED::APPLY:
          {
            ...
            do {
              ...
              if (is_rebase) {
                ...
                // current_segment)
                do_rebase<MACHO_T>(
                    static_cast<uint8_t>(DyldInfo::REBASE_TYPE::POINTER),
                ...
                do_bind<MACHO_T>(
                    DyldBindingInfo::CLASS::THREADED, th_bind_data.type,
  ...
  while (!done && stream_->pos() < end_offset) {
    ...
    switch (opcode) {
      ...
      case DyldInfo::BIND_OPCODES::SET_SYMBOL_TRAILING_FLAGS_IMM:
      {
        ...
        if (!str) {
          LIEF_ERR(
              "Can't read BIND_OPCODE_SET_SYMBOL_TRAILING_FLAGS_IMM symbol name"
          );
          break;
        }
        symbol_name = std::move(*str);
  ...
  while (stream_->pos() < end_offset) {
    ...
    switch (opcode) {
      ...
      case DyldInfo::BIND_OPCODES::SET_DYLIB_ORDINAL_ULEB:
      {
        auto val = stream_->read_uleb128();
        if (!val) {
          LIEF_ERR("Can't read BIND_OPCODE_SET_DYLIB_ORDINAL_ULEB uleb128 library "
                   "ordinal");
          break;
        }
      ...
      case DyldInfo::BIND_OPCODES::DO_BIND:
      {
        do_bind<MACHO_T>(DyldBindingInfo::CLASS::LAZY,
...
template<class MACHO_T>
ok_error_t BinaryParser::do_bind(DyldBindingInfo::CLASS cls, uint8_t type,
  ...
  auto binding_info =
      std::make_unique<DyldBindingInfo>(cls, DyldBindingInfo::TYPE(type), address,
                                        addend, ord, is_weak,
...
ok_error_t BinaryParser::do_rebase(uint8_t type, uint8_t segment_idx,
  ...
  if (section == nullptr) {
    LIEF_ERR("Can't find the section associated with the virtual address {:#x}",
             address);
    return make_error_code(lief_errors::not_found);
  }
  reloc->section_ = section;
...
ok_error_t BinaryParser::parse_chained_payload(SpanStream& stream) {
  ...
  if (!res_symbols_pools) {
    LIEF_WARN("Can't access the symbols pools "
              "(dyld_chained_fixups_header.symbols_offset)");
    return make_error_code(lief_errors::read_error);
  }
...
template<class MACHO_T>
ok_error_t BinaryParser::parse_chained_import(
  ...
  // Sanity checks according to dyld-852.2 /
  // MachOAnalyzer.cpp:forEachChainedFixupTarget
  if (header.imports_offset > stream.size() ||
      header.symbols_offset > stream.size())
  {
    LIEF_WARN("Malformed LC_DYLD_CHAINED_FIXUPS: "
              "dyld_chained_fixups_header.{{imports_offset, symbols_offset}} are "
              "out of ranges");
    return make_error_code(lief_errors::parsing_error);
  }

  if (header.imports_count >= MAX_BIND_ORDINAL) {
    LIEF_WARN("dyld_chained_fixups_header.imports_count is too large: {}. It "
              "should at most {}",
  ...
  switch (fmt) {
    ...
    case DYLD_CHAINED_FORMAT::IMPORT_ADDEND:
    {
      ...
      for (size_t i = 0; i < header.imports_count; ++i) {
        ...
        do_fixup<MACHO_T>(fmt, lib_ordinal, symbol_name, import.addend,
                          import.weak_import);
      }
      break;
    }
    ...
    default:
    {
      LIEF_WARN("Dyld Chained Fixups: {} is an unknown format",
...
ok_error_t BinaryParser::parse_chained_fixup(
  ...
  if (nb_segments > binary_->segments_.size()) {
    LIEF_WARN("Chained fixup: dyld_chained_starts_in_image.seg_count ({}) "
              ...
              starts.seg_count, binary_->segments_.size());
  ...
  for (uint32_t seg_idx = 0; seg_idx < nb_segments; ++seg_idx) {
    ...
    LIEF_DEBUG("    {:#08x} seg_offset[{}] = {} ({})",
               stream.pos() - sizeof(uint32_t), seg_idx, seg_info_offset,
               binary_->segments_[seg_idx]->name());
    if (seg_info_offset == 0) {
      struct DyldChainedFixups::chained_starts_in_segment info(
          0, {}, *binary_->segments_[seg_idx]
      );
    ...
    if (!parse_fixup_seg<MACHO_T>(stream, seg_info_offset, offset, seg_idx)) {
      LIEF_WARN("Error while parsing fixup in segment: {}",
                binary_->segments_[seg_idx]->name());
    }
  }
...
template<class MACHO_T>
ok_error_t BinaryParser::parse_fixup_seg(SpanStream& stream,
                                         uint32_t seg_info_offset, uint64_t offset,
                                         uint32_t seg_idx) {
  ...
  if (!res_seg_stream) {
    LIEF_ERR("Can't slice dyld_chained_starts_in_segment #{}: {}", seg_idx,
             to_string(get_error(res_seg_stream)));
  ...
  LIEF_DEBUG(
      "{}pointer_format    = {} ({})", DPREFIX, seg_info.pointer_format,
  ...
  struct DyldChainedFixups::chained_starts_in_segment info(seg_info_offset,
                                                           seg_info, *segment);
  ...
  for (uint32_t page_idx = 0; page_idx < seg_info.page_count; ++page_idx) {
    ...
    if (auto res = seg_stream.read<decltype(offset_in_page)>()) {
      ...
      LIEF_WARN("Can't read dyld_chained_starts_in_segment.page_start[{}]",
                page_idx);
      break;
    }
    ...
    if ((offset_in_page & DYLD_CHAINED_PTR_START_MULTI) > 0) {
      ...
      while (!chain_end) {
        ...
        const uint64_t off =
            page_start_off +
            overflow_index *
                sizeof(uint16_t); // &segInfo->page_start[overflowIndex]
        ...
        auto is_ok =
            walk_chain<MACHO_T>(*segment, chain_address, chain_offset, seg_info);
        if (!is_ok) {
          LIEF_WARN(
              "Error while walking through the chained fixup of the segment '{}'",
              segment->name()
          );
        }
        ++overflow_index;
...
template<class MACHO_T>
ok_error_t BinaryParser::do_fixup(DYLD_CHAINED_FORMAT fmt, int32_t ord,
                                  const std::string& symbol_name, int64_t addend,
                                  bool is_weak) {
...
template<class MACHO_T>
ok_error_t BinaryParser::walk_chain(
  ...
  while (!stop && !chain_end) {
    ...
    if (auto res = next_chain<MACHO_T>(/* in,out */ chain_address, chain_offset,
                                       seg_info))
    {
      chain_offset = *res;
    } else {
      LIEF_WARN("Error while computing the next chain for the offset: {:#x}",
                chain_offset);
      return make_error_code(lief_errors::parsing_error);
    }
...
template<class MACHO_T>
result<uint64_t> BinaryParser::next_chain(
    uint64_t& chain_address, uint64_t chain_offset,
    const details::dyld_chained_starts_in_segment& seg_info
) {
  const auto ptr_fmt =
      static_cast<DYLD_CHAINED_PTR_FORMAT>(seg_info.pointer_format);
...
template<class MACHO_T>
ok_error_t BinaryParser::process_fixup(
    SegmentCommand& segment, uint64_t chain_address, uint64_t chain_offset,
    const details::dyld_chained_starts_in_segment& seg_info
) {
  const auto ptr_fmt =
  ...
  switch (ptr_fmt) {
    ...
    case DYLD_CHAINED_PTR_FORMAT::PTR_ARM64E_USERLAND24:
    {
      ...
      auto is_ok =
          do_chained_fixup(segment, chain_address, chain_offset, seg_info, fixup);
      if (!is_ok) {
        LIEF_WARN("Can't process the fixup {} - {:#x}", segment.name(),
                  chain_offset);
    ...
    case DYLD_CHAINED_PTR_FORMAT::PTR_32_FIRMWARE:
    {
      LIEF_INFO("DYLD_CHAINED_PTR_FORMAT: {} is not implemented. Please consider "
...
ok_error_t BinaryParser::do_chained_fixup(
  ...
  if (fixup.auth_rebase.auth) {
    ...
    if (fixup.auth_bind.bind) {
      ...
      if (bind_ordinal >= chained_fixups_->internal_bindings_.size()) {
        LIEF_WARN("Out of range bind ordinal {} (max {})", bind_ordinal,
                  chained_fixups_->internal_bindings_.size());
        return make_error_code(lief_errors::read_error);
      }
      std::unique_ptr<ChainedBindingInfoList>& local_binding =
          chained_fixups_->internal_bindings_[bind_ordinal];
      local_binding->segment_ = &segment;
      local_binding->ptr_format_ = ptr_fmt;
      local_binding->set(fixup.auth_bind);

      chained_fixups_->all_bindings_.push_back(
          std::make_unique<ChainedBindingInfo>(*local_binding)
      );
      ...
      if (Symbol* sym = binding_extra_info->symbol()) {
        LIEF_DEBUG("{}[  BIND] {}@{:#x}: {} / sign ext: {:x}", DPREFIX,
                   segment.name(), address, sym->name(),
                   fixup.sign_extended_addend());
        return ok();
      }
      LIEF_DEBUG("{}[  BIND] {}@{:#x}: <missing symbol> / sign ext: {:x}", DPREFIX,
                 segment.name(), address, fixup.sign_extended_addend());
    ...
    LIEF_DEBUG("{}[REBASE] {}@{:#x}: {:#x}", DPREFIX, segment.name(), address,
               target);
  ...
  if (fixup.auth_bind.bind) {
    ...
    local_binding->ptr_format_ = ptr_fmt;
    ptr_fmt == DYLD_CHAINED_PTR_FORMAT::PTR_ARM64E_USERLAND24 ?
        local_binding->set(fixup.bind24) :
...
ok_error_t BinaryParser::do_chained_fixup(
  ...
  if (fixup.bind.bind > 0) {
    ...
    if (ordinal >= chained_fixups_->internal_bindings_.size()) {
      LIEF_WARN("Out of range bind ordinal {} (max {})", ordinal,
                chained_fixups_->internal_bindings_.size());
      return make_error_code(lief_errors::read_error);
    }

    std::unique_ptr<ChainedBindingInfoList>& local_binding =
        chained_fixups_->internal_bindings_[ordinal];
    local_binding->segment_ = &segment;
    local_binding->ptr_format_ = ptr_fmt;
    local_binding->set(fixup.bind);
   ...
   *  else
   *      newValue = (void*)((uintptr_t)this +
   * fixupLoc->generic64.unpackedTarget());
   *
...
ok_error_t BinaryParser::do_chained_fixup(
  ...
  if (fixup.bind.bind > 0) {
    ...
    if (Symbol* sym = binding_extra_info->symbol()) {
      LIEF_DEBUG("{}[  BIND] {}@{:#x}: {} / sign ext: {:x}", DPREFIX,
                 segment.name(), address, sym->name(), fixup.bind.addend);
      return ok();
    }
    LIEF_DEBUG("{}[  BIND] {}@{:#x}: <missing symbol> / sign ext: {:x}", DPREFIX,
               segment.name(), address, fixup.bind.addend);
    LIEF_ERR("Missing symbol for binding at ordinal {}", ordinal);
    return make_error_code(lief_errors::not_found);
  }
  // Rebase
...
ok_error_t BinaryParser::
    do_chained_fixup(SegmentCommand& /*segment*/, uint64_t /*chain_address*/,
...
ok_error_t BinaryParser::post_process(SymbolCommand& cmd) {
  ...
  SegmentCommand* nlist_linkedit =
      config_.from_dyld_shared_cache ?
          binary_->get_segment("__LINKEDIT") :
          binary_->segment_from_offset(cmd.symbol_offset());

  SegmentCommand* strings_linkedit =
      config_.from_dyld_shared_cache ?
          binary_->get_segment("__LINKEDIT") :
  ...
  if (nlist_linkedit == nullptr || strings_linkedit == nullptr) {
    ...
    if (!stream_->peek_data(strings_buffer, cmd.strings_offset(),
                            cmd.strings_size()))
    {
      return make_error_code(lief_errors::read_error);
    }
...
ok_error_t BinaryParser::post_process(FunctionStarts& cmd) {
  ...
  do {
    ...
    if (!val) {
      LIEF_WARN("Can't read value at offset: {:#012x} (#{} read)", stream.pos(),
                cmd.functions_.size());
      return make_error_code(lief_errors::read_error);
    }
...
ok_error_t BinaryParser::post_process(DynamicSymbolCommand& cmd) {
  ...
  for (const std::unique_ptr<Symbol>& sym : binary_->symbols_) {
    ...
    if (cmd.nb_local_symbols() > 0 && cmd.idx_local_symbol() <= isym &&
        isym < (cmd.idx_local_symbol() + cmd.nb_local_symbols()))
    {
    ...
    if (cmd.nb_external_define_symbols() > 0 &&
        ...
        isym <
            (cmd.idx_external_define_symbol() + cmd.nb_external_define_symbols()))
    {
      sym->category_ = Symbol::CATEGORY::EXTERNAL;
    }

    if (cmd.nb_undefined_symbols() > 0 && cmd.idx_undefined_symbol() <= isym &&
...
ok_error_t BinaryParser::post_process(LinkerOptHint& cmd) {
  ...
  if (linkedit == nullptr) {
    LIEF_WARN(
        "Can't find the segment that contains the LC_LINKER_OPTIMIZATION_HINT"
    );
    return make_error_code(lief_errors::not_found);
  }
  ...
  if (LinkEdit::segmentof(*linkedit)) {
    ...
    LIEF_WARN(
        "Weird: LC_LINKER_OPTIMIZATION_HINT is not in the __LINKEDIT segment"
    );
  }
  return ok();
}
  ...
  for (SegmentCommand& segment : binary_->segments()) {
    for (Section& section : segment.sections()) {
      ...
      for (size_t i = 0; i < count; ++i) {
        ...
        auto binding =
            std::make_unique<IndirectBindingInfo>(segment, sym, lib_ordinal, dylib,
                                                  addr);
...
template<class MACHO_T>
ok_error_t BinaryParser::parse_symtab(SymbolCommand& /*cmd*/, SpanStream& nlist_s,
                                      SpanStream& string_s) {
  using nlist_t = typename MACHO_T::nlist;
...
template<class MACHO_T>
ok_error_t BinaryParser::parse_data_in_code(DataInCode& cmd,
                                            BinaryStream& stream) {
...
ok_error_t BinaryParser::post_process(FunctionVariants& cmd) {
  ...
  if (linkedit == nullptr) {
    LIEF_WARN("Can't find the segment that contains the LC_FUNCTION_VARIANTS "
              "(offset={:#018x})",
              cmd.data_offset());
    return make_error_code(lief_errors::not_found);
  }
  ...
  if (LinkEdit::segmentof(*linkedit)) {
    ...
    LIEF_WARN("Weird: LC_FUNCTION_VARIANTS is not in the __LINKEDIT segment ({})",
              linkedit->name());
  }
...
ok_error_t BinaryParser::post_process(FunctionVariantFixups& cmd) {
  ...
  if (linkedit == nullptr) {
    LIEF_WARN("Can't find the segment that contains the "
              "LC_FUNCTION_VARIANT_FIXUPS (offset={:#018x})",
              cmd.data_offset());
    return make_error_code(lief_errors::not_found);
  }
  ...
  if (LinkEdit::segmentof(*linkedit)) {
    ...
    LIEF_WARN(
        "Weird: LC_FUNCTION_VARIANT_FIXUPS is not in the __LINKEDIT segment ({})",
        linkedit->name()
    );
  }
